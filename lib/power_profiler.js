...
PowerProfiler.prototype.update_device = function(mac, on) {
  ...
  db.hgetall(mac, function(err, device_data) {
    ...
    if (power_profile || device_data.manual_power) {
      if (on) {
        storage = parseFloat(device_data.manual_storage) || power_profile.storage || 0;
        power = parseFloat(device_data.manual_power) || power_profile.power;
